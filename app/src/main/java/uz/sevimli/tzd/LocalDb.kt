 ...
 * kontragentlar + mahsulotlar + shtrix kodlar. Internetsiz ham ishlaydi.
 */
class LocalDb private constructor(ctx: Context) :
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldV: Int, newV: Int) {
        if (oldV < 2) createProductTables(db)
    }
    ...
    private fun createProductTables(db: SQLiteDatabase) {
        db.execSQL(
            "CREATE TABLE IF NOT EXISTS product (" +
                ...
                "price INTEGER, uom TEXT, store_qty REAL, ms_updated TEXT)"
        )
        db.execSQL(
    ...
    /** Bir sahifa (bo'lak) mahsulotni bazaga yozadi (bor bo'lsa yangilaydi). */
    @Synchronized
        ...
        try {
            val pStmt = db.compileStatement(
                "INSERT OR REPLACE INTO product" +
                    "(moysklad_id,name,code,article,price,uom,store_qty,ms_updated) " +
                    "VALUES(?,?,?,?,?,?,?,?)")
            ...
            for (i in 0 until arr.length()) {
                ...
                pStmt.bindString(6, p.optString("uom"))
                pStmt.bindDouble(7, p.optDouble("store_qty", 0.0))
                pStmt.bindString(8, p.optString("ms_updated"))
                pStmt.executeInsert()
    ...
    /** Qoldiqlarni yangilaydi: {moysklad_id: qty}. Ro'yxatda yo'qlar 0 bo'ladi. */
    @Synchronized
    ...
    /** Delta uchun: bazadagi eng katta ms_updated. */
    @Synchronized
    ...
    private fun rowToProduct(
        ...
        price: Long, uom: String, qty: Double
    ...
    /** Shtrix bo'yicha mahsulot (topilmasa null). pack_qty ham qaytadi. */
    @Synchronized
    fun productByBarcode(bc: String): JSONObject? {
        readableDatabase.rawQuery(
            "SELECT p.moysklad_id,p.name,p.code,p.article,p.price,p.uom,p.store_qty,b.pack_qty " +
            ...
            val obj = rowToProduct(
                ...
                c.getLong(4), c.getString(5) ?: "", c.getDouble(6))
            if (!c.isNull(7)) obj.put("pack_qty", c.getDouble(7))
            return obj
        }
    }

    /** Kod yoki artikul bo'yicha mahsulot (topilmasa null). */
    @Synchronized
    fun productByCodeOrArticle(code: String): JSONObject? {
        readableDatabase.rawQuery(
            "SELECT moysklad_id,name,code,article,price,uom,store_qty FROM product " +
    ...
    /** Nomi/kodi/artikuli bo'yicha qidiruv. Javob: {products:[...]} (ProductSearch uchun). */
    @Synchronized
        ...
        readableDatabase.rawQuery(
            ...
            while (c.moveToNext()) {
                arr.put(JSONObject()
                    ...
                    .put("uom", c.getString(5) ?: "")
                    .put("store_qty", c.getDouble(6))
                    .put("barcode", ""))
            }
        }
